...
const samples = [
  {
    _id: new mongoose.Types.ObjectId(), status: 'ANALYSED', taskType: 'ESSAY', imagePath: '/private/a.png',
    errors: [{ written: 'pali', intended: 'pale', category: 'phonological', dismissed: false }],
  },
  {
    _id: new mongoose.Types.ObjectId(), status: 'REVIEWED', taskType: 'ESSAY', imagePath: '/private/b.png',
    errors: [{ written: 'playd', intended: 'played', category: 'morphological', dismissed: false }],
  },
  {
    _id: new mongoose.Types.ObjectId(), status: 'UPLOADED', taskType: 'ESSAY', imagePath: '/private/c.png',
...
test('student report uses all analysed samples and excludes identity and file paths', async () => {
  ...
  assert.deepEqual(report.basedOnSamples.map(String), samples.slice(0, 2).map(sample => String(sample._id)));
  assert.equal(report.strategies.length, 1);
  assert.deepEqual(received.errors.map(error => error.written), ['pali', 'playd']);

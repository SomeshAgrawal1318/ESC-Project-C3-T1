...
liveTest('live Azure context -> Gemini strategies -> approved worksheet PDF', {
  timeout: 300_000,
}, async t => {
  assert.ok(
  ...
  assert.equal(await readPdfSignature(response), '%PDF-', 'The Azure asset must contain PDF bytes');
